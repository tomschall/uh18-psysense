...
class Pycoproc:
    ...
    def __init__(self, i2c=None, sda='P22', scl='P21'):
        ...
        self.poke_memory(
            ADCON0_ADDR, (0x06 << _ADCON0_CHS_POSN) | _ADCON0_ADON_MASK)
    ...
    def poke_memory(self, addr, value):
        self._write(
            bytes([CMD_POKE, addr & 0xFF, (addr >> 8) & 0xFF, value & 0xFF]))
    ...
    def get_sleep_remaining(self):
        ...
        try:
            ...
            pass
        time_s = int((time_device_s / self.clk_cal_factor) +
                     0.5)  # 0.5 used for round
        return time_s
    ...
    def go_to_sleep(self, gps=True):
        ...
        if self.wake_int_pin:
            if self.wake_int_pin_rising_edge:
                ...
                self.mask_bits_in_memory(OPTION_REG_ADDR, ~(
                    1 << 6))  # falling edge of INT pin
